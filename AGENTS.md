...
- Per `make graphify-wiki-docs`, usa il target `make` dedicato: applica gia `GRAPHIFY_OPENAI_MODEL=$(GRAPHIFY_DOC_MODEL)`, `--max-concurrency 1` e `--api-timeout 180` per evitare l'hang osservato con `gpt-5.5` sul path docs di Graphify.
- Per i target `*-docs`, il default operativo e la variabile unica `GRAPHIFY_DOC_MODEL`, oggi `gpt-reserve`: su Graphify privilegiamo stabilita, costo e latenza rispetto alla massima qualita del modello, perche l'estrazione semantica dei corpus docs e un carico batch ripetitivo. Il precedente `gpt-5.4-mini` e stato ritirato dal codex-lb il 2026-09-09 (HTTP 503 `no_plan_support_for_model`); evita `gpt-5.5` sui target docs che hanno gia mostrato hang o timeout.
- Un modello docs che non risponde NON fa fallire il target: `graphify extract` degrada a `partial results` ed esce comunque 0, riscrivendo `graph.json` senza arricchimento semantico. Dopo un aggiornamento docs verifica sempre la presenza di `chunk N/N done` e l'assenza di `WARNING: ... semantic chunk(s) failed`, non solo l'exit code.
