...
from app.modules.inaz.schemas import InazAutoSyncConfigUpdate
from app.modules.inaz.services.auto_sync import (
...
def _create_credential(db: Session, user: ApplicationUser, *, active: bool = True) -> InazCredential:
...
def test_get_auto_sync_config_creates_default_row() -> None:
    ...
    try:
        ...
        assert db.get(InazAutoSyncConfig, 1) is not None
    finally:
        db.close()
...
def test_update_auto_sync_config_can_store_disabled_state_without_credential() -> None:
    ...
    try:
        config = update_auto_sync_config(
            db,
            InazAutoSyncConfigUpdate(job_enabled=False, collaborator_limit=25),
            user_id=user.id,
        )
        assert config.job_enabled is False
...
def test_update_auto_sync_config_rejects_unknown_credential() -> None:
    ...
    try:
        with pytest.raises(HTTPException) as excinfo:
            update_auto_sync_config(
                db,
                InazAutoSyncConfigUpdate(job_enabled=True, credential_id=99999),
                user_id=user.id,
            )
...
def test_update_auto_sync_config_rejects_disabled_credential() -> None:
    ...
    try:
        ...
        with pytest.raises(HTTPException) as excinfo:
            update_auto_sync_config(
                db,
                InazAutoSyncConfigUpdate(job_enabled=True, credential_id=credential.id),
                user_id=user.id,
            )
...
def test_update_auto_sync_config_rejects_enable_without_credential() -> None:
    ...
    try:
        with pytest.raises(HTTPException) as excinfo:
            update_auto_sync_config(
                db,
                InazAutoSyncConfigUpdate(job_enabled=True),
                user_id=user.id,
            )
        assert excinfo.value.status_code == 409
    finally:
...
def test_update_auto_sync_config_can_clear_credential_when_disabled() -> None:
    ...
    try:
        ...
        cleared = update_auto_sync_config(
            db,
            InazAutoSyncConfigUpdate(job_enabled=False, credential_id=None),
            user_id=user.id,
        )
...
def test_resolve_trigger_user_id_prefers_config_updated_by_user() -> None:
    ...
    try:
        ...
        config = InazAutoSyncConfig(id=1, job_enabled=True, credential_id=credential.id, updated_by_user_id=owner.id)
...
def test_resolve_trigger_user_id_falls_back_to_credential_owner() -> None:
    ...
    try:
        ...
        config = InazAutoSyncConfig(id=1, job_enabled=True, credential_id=credential.id, updated_by_user_id=None)
        user_id = _resolve_trigger_user_id(db, config, credential)
        assert user_id == owner.id
    finally:
        db.close()
...
def test_resolve_trigger_user_id_uses_first_active_inaz_user_when_needed() -> None:
    ...
    no_module = _create_user("auto_sync_no_module", module_inaz=False)
    ...
    try:
        ...
        db.add(credential)
        db.commit()
        config = InazAutoSyncConfig(id=1, job_enabled=True, credential_id=credential.id, updated_by_user_id=inactive.id)
        user_id = _resolve_trigger_user_id(db, config, credential)
...
def test_resolve_trigger_user_id_raises_when_no_active_inaz_user_exists() -> None:
    ...
    no_module = _create_user("auto_sync_none_no_module", module_inaz=False)
    db = TestingSessionLocal()
    try:
...
def test_trigger_auto_sync_job_skips_when_running_job_exists(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        db.add(config)
        db.commit()
        monkeypatch.setattr("app.modules.inaz.services.auto_sync.has_running_sync_job", lambda db: True)
        assert trigger_auto_sync_job(db) is None
    finally:
        db.close()
...
def test_trigger_auto_sync_job_persists_failure_when_worker_launch_crashes(
    ...
    try:
        ...
        db.commit()
        monkeypatch.setattr("app.modules.inaz.services.auto_sync.has_running_sync_job", lambda db: False)
        monkeypatch.setattr("app.modules.inaz.services.auto_sync.get_sync_artifact_dir", lambda job_id: tmp_path / job_id)

        def _boom(job: InazSyncJob) -> int:
            raise RuntimeError("worker boom")

        monkeypatch.setattr("app.modules.inaz.services.auto_sync.launch_sync_worker", _boom)

        with pytest.raises(RuntimeError):
            trigger_auto_sync_job(db)

        jobs = db.query(InazSyncJob).all()
        assert len(jobs) == 1
...
def test_trigger_auto_sync_job_uses_current_month_and_creates_artifacts(
    ...
    try:
        ...
        monkeypatch.setattr("app.modules.inaz.services.auto_sync.has_running_sync_job", lambda db: False)
        monkeypatch.setattr("app.modules.inaz.services.auto_sync.get_sync_artifact_dir", lambda job_id: tmp_path / job_id)
        monkeypatch.setattr("app.modules.inaz.services.auto_sync.launch_sync_worker", lambda job: 9090)
        ...
        assert Path(job.json_artifact_path or "").name == "inaz_collaboratori.json"
