...
from app.modules.presenze.schemas import PresenzeBankHoursGuidanceConfigUpdate
...
def test_get_bank_hours_guidance_config_creates_default_row() -> None:
    ...
    try:
        ...
        assert config.id == 1
        assert db.get(PresenzeBankHoursGuidanceConfig, 1) is not None
    finally:
        db.close()
...
def test_serialize_bank_hours_guidance_revision_falls_back_to_username() -> None:
    ...
    try:
        ...
        db.add(PresenzeBankHoursGuidanceConfig(id=1))
        db.add(revision)
        db.commit()
        db.refresh(revision)
...
def test_list_bank_hours_guidance_config_revisions_returns_newest_first() -> None:
    ...
    try:
        config = get_bank_hours_guidance_config(db)
        older = PresenzeBankHoursGuidanceConfigRevision(
            config_id=config.id,
            allow_derived_profile=False,
            include_overtime_day=True,
            include_overtime_night=True,
        ...
        newer = PresenzeBankHoursGuidanceConfigRevision(
            config_id=config.id,
...
def test_update_bank_hours_guidance_config_noop_does_not_create_revision() -> None:
    ...
    try:
        ...
        assert db.query(PresenzeBankHoursGuidanceConfigRevision).count() == 0
    finally:
        db.close()
