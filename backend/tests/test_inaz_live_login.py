...
from app.modules.inaz.services import live_login
...
def _install_fake_live_login_modules(
    ...
    *,
    ...
    scraper_root = tmp_path / "inaz-scraper"
    scraper_src = scraper_root / "src"
    scraper_src.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(live_login.settings, "inaz_scraper_project_path", str(scraper_root))
    ...
    cli_module = ModuleType("inaz_scraper.cli")
    ...
    collaborators_module = ModuleType("inaz_scraper.collaborators")
    ...
    inaz_scraper_module = ModuleType("inaz_scraper")
    inaz_scraper_module.cli = cli_module
    inaz_scraper_module.collaborators = collaborators_module
    ...
    monkeypatch.setitem(live_login.sys.modules, "inaz_scraper", inaz_scraper_module)
    monkeypatch.setitem(live_login.sys.modules, "inaz_scraper.cli", cli_module)
    monkeypatch.setitem(live_login.sys.modules, "inaz_scraper.collaborators", collaborators_module)
...
def test_ensure_scraper_src_on_path_inserts_once_and_rejects_missing_project(
    ...
    monkeypatch.setattr(live_login.settings, "inaz_scraper_project_path", str(missing_root))

    with pytest.raises(RuntimeError, match="Inaz scraper project not found"):
