...
def test_night_shift_breakdown_distinguishes_ordinary_and_overtime_minutes() -> None:
    collaborator = InazCollaborator(id=uuid.uuid4(), employee_code="3001", company_code="53", name="Turnista")
    template = InazScheduleTemplate(id=23, code="TURNO_NOTTE", label="Turno notte", is_active=True)
    assignment = InazCollaboratorScheduleAssignment(collaborator_id=collaborator.id, template_id=template.id)
    rule = InazScheduleRule(
        template_id=template.id,
        weekday=0,
    ...
    context = _context(collaborator, assignment, template, [rule])
    record = InazDailyRecord(id=uuid.uuid4(), collaborator_id=collaborator.id, work_date=date(2026, 5, 18))
    punches = [InazDailyPunch(daily_record_id=record.id, sequence=1, entry_time=time(22, 0), exit_time=time(3, 0))]
...
def test_festive_night_shift_breakdown_tracks_festive_night_buckets() -> None:
    collaborator = InazCollaborator(id=uuid.uuid4(), employee_code="3002", company_code="53", name="Turnista festivo")
    template = InazScheduleTemplate(id=24, code="TURNO_NOTTE_FEST", label="Turno notte festivo", is_active=True)
    assignment = InazCollaboratorScheduleAssignment(collaborator_id=collaborator.id, template_id=template.id)
    rule = InazScheduleRule(
        template_id=template.id,
        weekday=1,
...
def test_archivio_summary_writes_ccnl_breakdown_columns() -> None:
    collaborator = InazCollaborator(id=uuid.uuid4(), employee_code="1854", company_code="53", name="Operaio")
    template = InazScheduleTemplate(id=33, code="CCNL_SUM", label="Riepilogo CCNL", is_active=True)
    assignment = InazCollaboratorScheduleAssignment(collaborator_id=collaborator.id, template_id=template.id)
    ferial_night_rule = InazScheduleRule(
        template_id=template.id,
        weekday=0,
        recurrence_kind="weekly",
        start_time=time(22, 0),
    ...
    festive_night_rule = InazScheduleRule(
        template_id=template.id,
        weekday=1,
        recurrence_kind="weekly",
        start_time=time(22, 0),
    ...
    festive_record = InazDailyRecord(
        id=uuid.uuid4(),
        collaborator_id=collaborator.id,
    ...
    punches = {
        str(ferial_record.id): [InazDailyPunch(daily_record_id=ferial_record.id, sequence=1, entry_time=time(22, 0), exit_time=time(3, 0))],
        str(festive_record.id): [InazDailyPunch(daily_record_id=festive_record.id, sequence=1, entry_time=time(22, 0), exit_time=time(3, 0))],
    }
...
def test_build_schedule_context_sorts_assignments_and_rules() -> None:
    collaborator = InazCollaborator(id=uuid.uuid4(), employee_code="1854", company_code="53", name="Operaio")
    template = InazScheduleTemplate(id=50, code="CTX", label="Context", is_active=True)
    old_assignment = InazCollaboratorScheduleAssignment(
        id=1,
        collaborator_id=collaborator.id,
        template_id=template.id,
        valid_from=date(2026, 1, 1),
    )
    new_assignment = InazCollaboratorScheduleAssignment(
        id=2,
        collaborator_id=collaborator.id,
        template_id=template.id,
        valid_from=date(2026, 2, 1),
    )
    late_rule = InazScheduleRule(
        id=2,
        template_id=template.id,
        weekday=0,
        recurrence_kind="weekly",
    ...
    holiday = InazHoliday(holiday_date=date(2026, 5, 1), label="Festa", company_code="53", holiday_kind="ordinary")
    db = _db()
    try:
...
def test_classify_daily_record_returns_imported_when_context_missing_or_unusable() -> None:
    collaborator = InazCollaborator(id=uuid.uuid4(), employee_code="1854", company_code="53", name="Operaio")
    record = InazDailyRecord(id=uuid.uuid4(), collaborator_id=collaborator.id, work_date=date(2026, 5, 18), ordinary_minutes=120, straordinario_minutes=30)
    ...
    template = InazScheduleTemplate(id=51, code="CTX", label="Context", is_active=False)
    assignment = InazCollaboratorScheduleAssignment(collaborator_id=collaborator.id, template_id=template.id)
    context = _context(collaborator, assignment, template, [])
    punches = [InazDailyPunch(daily_record_id=record.id, sequence=1, entry_time=time(7, 0), exit_time=time(8, 0))]
...
def test_compute_minutes_helpers_support_incomplete_and_overnight_segments() -> None:
    ...
    punches = [
        InazDailyPunch(daily_record_id=record_id, sequence=1, entry_time=time(22, 0), exit_time=time(1, 0)),
        InazDailyPunch(daily_record_id=record_id, sequence=2, entry_time=None, exit_time=time(2, 0)),
    ]
    rules = [InazScheduleRule(template_id=1, weekday=0, recurrence_kind="weekly", start_time=time(23, 0), end_time=time(0, 0))]
...
def test_compute_minutes_helpers_ignore_zero_duration_segments() -> None:
    record_id = uuid.uuid4()
    punches = [InazDailyPunch(daily_record_id=record_id, sequence=1, entry_time=time(7, 0), exit_time=time(7, 0))]
...
def test_resolve_assignment_and_template_date_bounds_are_enforced() -> None:
    collaborator = InazCollaborator(id=uuid.uuid4(), employee_code="1854", company_code="53", name="Operaio")
    template = InazScheduleTemplate(id=52, code="TMP", label="Template", is_active=True, valid_from=date(2026, 5, 10), valid_to=date(2026, 5, 20))
    assignment = InazCollaboratorScheduleAssignment(
        collaborator_id=collaborator.id,
        template_id=template.id,
...
def test_rule_matching_supports_recurrence_edge_cases_and_seasons() -> None:
    weekly = InazScheduleRule(template_id=1, weekday=0, recurrence_kind="weekly", start_time=time(7, 0), end_time=time(8, 0))
    first = InazScheduleRule(template_id=1, weekday=0, recurrence_kind="first_weekday_of_month", start_time=time(7, 0), end_time=time(8, 0))
    third = InazScheduleRule(template_id=1, weekday=0, recurrence_kind="nth_weekday_of_month", week_of_month=3, start_time=time(7, 0), end_time=time(8, 0))
    alternating_no_anchor = InazScheduleRule(template_id=1, weekday=0, recurrence_kind="alternating_weeks", start_time=time(7, 0), end_time=time(8, 0))
    alternating_future_anchor = InazScheduleRule(
        template_id=1,
        weekday=0,
        recurrence_kind="alternating_weeks",
    ...
    unknown = InazScheduleRule(template_id=1, weekday=0, recurrence_kind="unsupported", start_time=time(7, 0), end_time=time(8, 0))
    seasonal = InazScheduleRule(
        template_id=1,
        weekday=0,
        recurrence_kind="weekly",
        start_time=time(7, 0),
...
def test_scheduled_minutes_for_day_handles_missing_context_assignment_and_template() -> None:
    ...
    missing_template_assignment = InazCollaboratorScheduleAssignment(collaborator_id=collaborator.id, template_id=999)
...
def test_resolve_holiday_prefers_context_then_defaults() -> None:
    collaborator = InazCollaborator(id=uuid.uuid4(), employee_code="1854", company_code="53", name="Operaio")
    context_holiday = InazHoliday(holiday_date=date(2026, 9, 8), label="Locale", company_code="53", holiday_kind="ordinary")
...
def test_seed_holidays_for_year_creates_missing_rows_only() -> None:
    ...
    try:
        ...
        stored = db.query(InazHoliday).filter(InazHoliday.holiday_date == date(2026, 12, 25)).one_or_none()
