...
import app.modules.inaz.services.sync_runtime as sync_runtime_module
from app.modules.inaz.services import sync_worker
...
class _FakeDb:
    ...
    def get(self, model, key):
        if model is sync_worker.InazSyncJob:
            return self.job
        if model is sync_worker.InazImportJob:
...
def test_main_completes_job_and_writes_artifacts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def fake_create_import_job(current_db, *, parsed, requested_by_user_id, filename, params_json):
        ...
        assert filename == "inaz_collaboratori.json"
    ...
    def fake_run_scrape_with_credentials(**kwargs):
        ...
        return {
            "authenticated_url": "https://inaz/auth",
    ...
    assert used_credentials == [(9, "https://inaz/auth")]
...
def test_main_fails_when_requested_user_is_missing(
    ...
    assert exit_code == 1
    assert job.status == "failed"
    assert job.error_detail == "Requested by user not found for Inaz sync job"
    assert import_job.status == "failed"
...
def test_main_fails_when_credential_mode_is_legacy(
    ...
    assert exit_code == 1
    assert job.status == "failed"
    assert "Legacy Inaz sync mode is disabled" in (job.error_detail or "")
    assert import_job.status == "failed"
...
def test_module_main_guard_raises_system_exit(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit, match="2"):
        runpy.run_module("app.modules.inaz.services.sync_worker", run_name="__main__")
