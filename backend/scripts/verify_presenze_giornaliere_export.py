...
from app.modules.presenze.services.schedule_engine import build_schedule_context, scheduled_minutes_for_day
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-xlsm",
        help="File XLSM di output. Default: ./tmp/presenze_giornaliere_<YYYY-MM>.xlsm",
    )
    parser.add_argument(
        "--report-csv",
...
def load_export_rows(db, period_start: date, employee_codes: list[str] | None) -> tuple[list[PresenzeCollaborator], list[ExportTimesheetRow]]:
    stmt = select(PresenzeCollaborator).order_by(PresenzeCollaborator.employee_code.asc())
    if employee_codes:
    ...
    for collaborator in collaborators:
        ...
        if not daily_rows:
            continue
        punches = db.execute(select(PresenzeDailyPunch).where(PresenzeDailyPunch.daily_record_id.in_([item.id for item in daily_rows]))).scalars().all()
...
def main() -> int:
    ...
    output_xlsm = (
        ...
        if args.output_xlsm
        else REPO_ROOT / "tmp" / f"presenze_giornaliere_{period_start:%Y-%m}.xlsm"
    )
