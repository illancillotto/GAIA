...
class CatastoCredentialLease(Base):
    __tablename__ = "catasto_credential_leases"

    sister_username: Mapped[str] = mapped_column(String(128), primary_key=True)
    credential_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("catasto_credentials.id", ondelete="CASCADE"), nullable=False, index=True
    )
    batch_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("catasto_batches.id", ondelete="CASCADE"), nullable=False, index=True
    )
