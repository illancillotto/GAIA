...
def get_tree(db: Annotated[Session, Depends(get_db)]) -> list[OrgUnitTreeNode]:
    return svc.build_tree(db)
...
def get_unit_detail(unit_id: UUID, db: Annotated[Session, Depends(get_db)]) -> UnitDetailResponse:
    detail = svc.get_unit_detail(db, unit_id)
    if detail is None:
...
def create_unit(
    ...
    unit = repo.create_unit(db, payload, user_id=current_user.id)
...
def update_unit(
    ...
    if payload.parent_id is not None:
        ...
        if repo.get_unit(db, payload.parent_id) is None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Parent unit not found")
