...
class AnagraficaVisuraRoutingAnomaly(Base):
    __tablename__ = "ana_visure_routing_anomalies"

    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    source_path: Mapped[str] = mapped_column(String(1024), nullable=False, unique=True, index=True)
    filename: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    identifier: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    identifier_kind: Mapped[str | None] = mapped_column(String(16), nullable=True)
