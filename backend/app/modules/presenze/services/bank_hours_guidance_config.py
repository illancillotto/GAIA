...
from app.modules.presenze.models import PresenzeBankHoursGuidanceConfig, PresenzeBankHoursGuidanceConfigRevision
from app.modules.presenze.schemas import PresenzeBankHoursGuidanceConfigResponse, PresenzeBankHoursGuidanceConfigRevisionResponse, PresenzeBankHoursGuidanceConfigUpdate


def get_bank_hours_guidance_config(db: Session) -> PresenzeBankHoursGuidanceConfig:
    config = db.get(PresenzeBankHoursGuidanceConfig, 1)
    if config is not None:
        return config

    config = PresenzeBankHoursGuidanceConfig(
        id=1,
        allow_derived_profile=settings.presenze_bank_hours_guidance_allow_derived_profile,
        include_overtime_day=settings.presenze_bank_hours_guidance_include_overtime_day,
        include_overtime_night=settings.presenze_bank_hours_guidance_include_overtime_night,
        include_overtime_festive=settings.presenze_bank_hours_guidance_include_overtime_festive,
        include_overtime_festive_night=settings.presenze_bank_hours_guidance_include_overtime_festive_night,
        min_suggested_minutes=settings.presenze_bank_hours_guidance_min_suggested_minutes,
    )
    db.add(config)
    db.flush()
    return config
...
def serialize_bank_hours_guidance_config_with_user(
    ...
    return PresenzeBankHoursGuidanceConfigResponse(
...
def serialize_bank_hours_guidance_revision(
    db: Session,
    revision: PresenzeBankHoursGuidanceConfigRevision,
) -> PresenzeBankHoursGuidanceConfigRevisionResponse:
    return PresenzeBankHoursGuidanceConfigRevisionResponse(
        id=revision.id,
...
def list_bank_hours_guidance_config_revisions(db: Session) -> list[PresenzeBankHoursGuidanceConfigRevision]:
    return (
        db.query(PresenzeBankHoursGuidanceConfigRevision)
        .order_by(
            PresenzeBankHoursGuidanceConfigRevision.changed_at.desc(),
            PresenzeBankHoursGuidanceConfigRevision.id.desc(),
        )
        .all()
    )
