...
def build_preview(parsed: ParsedImportPayload) -> PresenzeImportPreviewResponse:
    return PresenzeImportPreviewResponse(
...
def import_collaborator_payload(
    db: Session,
    *,
    ...
    for daily_row in payload.daily_rows:
        ...
        record = db.execute(
            select(PresenzeDailyRecord).where(
                ...
                PresenzeDailyRecord.work_date == work_date,
            )
        ).scalar_one_or_none()
        if record is None:
        ...
        db.query(PresenzeDailyPunch).filter(PresenzeDailyPunch.daily_record_id == record.id).delete()
    ...
    db.query(PresenzeEventSummary).filter(
        ...
        PresenzeEventSummary.period_end == payload.period_end,
    ).delete()
...
def run_import_job(
    ...
    *,
    ...
    try:
        ...
        db.commit()
        db.refresh(job)
        return PresenzeImportJsonResponse(job=PresenzeImportJobResponse.model_validate(job), preview=preview)
...
def upsert_collaborator(db: Session, payload: Any) -> PresenzeCollaborator:
    ...
    collaborator = db.execute(
        select(PresenzeCollaborator).where(
            PresenzeCollaborator.employee_code == employee_code,
            PresenzeCollaborator.company_code == company_code,
        )
    ).scalar_one_or_none()
    if collaborator is None:
        collaborator = PresenzeCollaborator(employee_code=employee_code, company_code=company_code, name=employee_code)
