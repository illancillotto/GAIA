...
def _mark_job_cancelled(job_id: str) -> None:
    ...
    try:
        ...
        job.error_detail = "Export XLSM job cancelled by user"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a GAIA Presenze XLSM export job in a separate process.")
...
def _parse_period_start(raw_value: str | None) -> date:
    if not raw_value:
        raise RuntimeError("Missing period_start in export XLSM job params")
...
def main() -> int:
    ...
    try:
        ...
        if job is None:
            print(f"Presenze export job {args.job_id} not found", file=sys.stderr)
            return 2

        params = dict(job.params_json or {})
        if params.get("mode") != "export_xlsm":
            raise RuntimeError("Unsupported job mode for xlsm export worker")

        artifact_dir = get_sync_artifact_dir(str(job.id))
        artifact_dir.mkdir(parents=True, exist_ok=True)
        progress_path = artifact_dir / "progress.json"
        summary_path = artifact_dir / "summary.json"
        output_path = artifact_dir / "giornaliere_export.xlsm"
        ...
        resolved_employee_kind = generate_xlsm_export(
            db,
            period_start=_parse_period_start(params.get("period_start")),
            collaborator_ids=[uuid.UUID(item) for item in params.get("collaborator_ids", [])] if params.get("collaborator_ids") else None,
            employee_kind=params.get("employee_kind"),
        ...
        final_params["resolved_employee_kind"] = resolved_employee_kind
        ...
        _write_json(
            summary_path,
            {
                ...
                "resolved_employee_kind": resolved_employee_kind,
