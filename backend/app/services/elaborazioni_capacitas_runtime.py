...
from app.services.elaborazioni_capacitas_domande_irrigue import get_domande_irrigue_sync_job, run_domande_irrigue_sync_job
...
async def run_domande_irrigue_job_by_id(job_id: int) -> None:
    db = SessionLocal()
    manager: CapacitasSessionManager | None = None
    credential_id: int | None = None
    try:
        ...
        await run_domande_irrigue_sync_job(db, client, scraper, job)
        mark_credential_used(db, credential.id)
    except Exception as exc:
        logger.exception("Errore worker job domande irrigue Capacitas: job_id=%d err=%s", job_id, exc)
        db.rollback()
        if credential_id is not None:
            mark_credential_error(db, credential_id, str(exc))
        job = get_domande_irrigue_sync_job(db, job_id)
