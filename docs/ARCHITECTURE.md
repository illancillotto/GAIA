...
- Il frontend raggruppa i risultati nell'ordine operativo `Utenze`, `Ruolo`, `Catasto`: una ricerca per nome, cognome, codice fiscale o partita IVA mostra quindi prima l'utenza collegata e solo dopo avvisi di pagamento e risultati catastali. Il ranking interno di ciascun gruppo resta quello restituito dall'API, che antepone i soggetti anagrafici (`subject_person`, `subject_company`) agli altri match dello stesso modulo.
...
- La stessa logica e disponibile come componente riusabile `OperationalSearchBox`: la home usa la variante `hero`, mentre la `Topbar` delle pagine dentro `AppShell` usa la variante compatta con scorciatoia `Ctrl/Cmd+K`. Entrambe riconoscono coordinate GIS, applicano le stesse scorciatoie, aprono direttamente il risultato univoco e mantengono la priorita `Utenze`, `Ruolo`, `Catasto`.
- Quando la query produce piu risultati o risultati misti, sia home sia topbar aprono la stessa modale estesa e aumentano il limite API da `8` a `30`. La SERP `GET /search?q=...` resta disponibile tramite il pulsante `Vista estesa`: mostra form, conteggi, chip `Tutto`/`Utenze`/`Ruolo`/`Catasto` e card navigabili.
- Il vecchio cruscotto a card e stato ridotto a una riga secondaria `Stato operativo`, alimentata dai summary gia disponibili di Utenze, Ruolo, Catasto e NAS: utenti in anagrafica, anagrafiche anomale, annualita ruolo caricate, particelle a ruolo, particelle `FD`/fuori distretto, distretti ruolo effettivi esclusi `FD` e raggruppamenti non classificati, e dati NAS.
