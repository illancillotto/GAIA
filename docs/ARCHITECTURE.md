...
- La stessa logica e disponibile come componente riusabile `OperationalSearchBox`: la home usa la variante `hero`, mentre la `Topbar` delle pagine dentro `AppShell` usa la variante compatta con scorciatoia `Ctrl/Cmd+K`, cosi l'operatore puo cercare utenze, ruolo e catasto senza tornare alla home.
- Quando la query produce piu risultati o risultati misti, il dropdown apre la SERP `GET /search?q=...`: la pagina mostra una vista ampia con form, conteggi, chip `Tutto`/`Utenze`/`Ruolo`/`Catasto` e card navigabili. `Enter` apre direttamente solo quando esiste un risultato univoco; negli altri casi porta alla SERP, come il pulsante `Vedi tutti i risultati`.
