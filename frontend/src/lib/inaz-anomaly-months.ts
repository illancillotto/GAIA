...
export function recordHasAnomaly(record: Pick<InazDailyRecord, "detail_anomalies" | "detail_error">): boolean {
  return record.detail_anomalies.length > 0 || Boolean(record.detail_error);
}

export function countAnomaliesInRecords(records: InazDailyRecord[]): number {
