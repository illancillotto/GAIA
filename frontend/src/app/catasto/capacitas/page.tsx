...
export default function CatastoCapacitasPage() {
    ...
    >
      <div className="space-y-6">
        <CatastoHero
          ...
          title="Ricerca anagrafica sul portale inVOLTURE usando il pool account del modulo Catasto."
          ...
          actions={
            error ? (
              <CatastoNoticeCard title="Errore ricerca" description={error} tone="danger" />
            ) : (
              <>
                <CatastoNoticeCard
                ...
                />
                <Link className="btn-secondary" href="/catasto/settings">
                  Apri Credenziali
                </Link>
              </>
            )
        ...
        >
          <div className="grid gap-3 sm:grid-cols-3">
            <CatastoMiniStat eyebrow="Pool" value={`${activeCredentialsCount}/${credentials.length}`} description="Account attivi sul totale configurato." />
            <CatastoMiniStat eyebrow="Ricerca corrente" value={results?.total ?? 0} description="Record restituiti dall'ultima ricerca." tone={results && results.total > 0 ? "success" : "default"} />
            <CatastoMiniStat eyebrow="Account forzato" value={activeCredential ? activeCredential.label : "Auto"} description="Se impostato, il backend non esegue auto-selezione." />
          </div>
        </CatastoHero>
