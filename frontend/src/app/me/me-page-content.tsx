...
export type MeTabKey = "overview" | "presenze" | "operativita" | "dotazioni" | "anomalie";
...
function getPathForTab(tab: MeTabKey): string {
  if (tab === "overview") return "/me";
  return `/me/${tab}`;
}

export default function MePageContent({ initialTab = "overview" }: { initialTab?: MeTabKey }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [activeTab, setActiveTab] = useState<MeTabKey>(initialTab);
  ...
  useEffect(() => {
    if (typeof window === "undefined" || pathname !== "/me") return;
    const legacyTab = getTabFromHash(window.location.hash);
    if (legacyTab === "overview") return;
    const query = searchParams.toString();
    const targetPath = getPathForTab(legacyTab);
  ...
  const navigateToTab = (tab: MeTabKey) => {
    const query = new URLSearchParams(searchParams.toString());
    const targetPath = getPathForTab(tab);
    const target = query.toString() ? `${targetPath}?${query.toString()}` : targetPath;
      ...
      <div className="page-stack">
        ...
        <article className="panel-card">
          <div className="flex flex-wrap items-center gap-3">
            <button className={activeTab === "overview" ? "btn-primary" : "btn-secondary"} type="button" onClick={() => navigateToTab("overview")}>
              Panoramica
            </button>
            <button className={activeTab === "presenze" ? "btn-primary" : "btn-secondary"} type="button" onClick={() => navigateToTab("presenze")}>
              Presenze
            </button>
            <button className={activeTab === "operativita" ? "btn-primary" : "btn-secondary"} type="button" onClick={() => navigateToTab("operativita")}>
              Operatività
            </button>
            <button className={activeTab === "dotazioni" ? "btn-primary" : "btn-secondary"} type="button" onClick={() => navigateToTab("dotazioni")}>
              Dotazioni
            </button>
            <button className={activeTab === "anomalie" ? "btn-primary" : "btn-secondary"} type="button" onClick={() => navigateToTab("anomalie")}>
              Anomalie
            </button>
