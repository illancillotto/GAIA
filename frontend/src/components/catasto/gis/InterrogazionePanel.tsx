...
export default function InterrogazionePanel(state: InterrogazioneState) {
  if (!state.open) {
  ...
  return (
    <aside aria-label="Interrogazione territoriale" className="absolute bottom-3 right-3 top-3 z-30 flex w-[min(28rem,calc(100%-1.5rem))] flex-col rounded-2xl border border-emerald-950/15 bg-[#fffdf6]/95 shadow-2xl backdrop-blur">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
        ...
        <button type="button" disabled className="w-full rounded-xl bg-stone-200 px-4 py-3 text-sm font-bold text-stone-500">Scheda territoriale disponibile da M24</button>
      </div>
    </aside>
  );
}
