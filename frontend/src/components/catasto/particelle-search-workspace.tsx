...
type ParticelleSearchWorkspaceProps = {
  mode?: "all" | "ruolo-only";
};
...
const RUOLO_ONLY_FILTERS: SearchFilters = {
  ...DEFAULT_FILTERS,
  soloConAnagrafica: false,
  soloARuolo: true,
};

export function ParticelleSearchWorkspace({ mode = "all" }: ParticelleSearchWorkspaceProps) {
  const isRuoloOnly = mode === "ruolo-only";
  const [selectedParticella, setSelectedParticella] = useState<CatParticella | null>(null);
  const [filters, setFilters] = useState<SearchFilters>(isRuoloOnly ? RUOLO_ONLY_FILTERS : DEFAULT_FILTERS);
  ...
  const emptyDescription = isRuoloOnly
    ? "Non ci sono particelle a ruolo per i filtri correnti. Se il risultato resta vuoto anche senza altri filtri, verifica che l'archivio Ruolo sia stato importato e collegato al Catasto."
    : filters.soloARuolo
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                {isRuoloOnly
                  ? "La ricerca e limitata alle particelle correnti collegate al ruolo consortile (`is_current=true`, `solo_a_ruolo=true`)."
                  : "I filtri attuali sono applicati su particelle correnti (`is_current=true`)."}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                disabled={busy}
                onClick={() => {
                  const resetFilters = isRuoloOnly ? RUOLO_ONLY_FILTERS : DEFAULT_FILTERS;
