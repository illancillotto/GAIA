...
import { importFuelCards } from "@/features/operazioni/api/client";

type ImportFuelCardsModalProps = {
  open: boolean;
...
export function ImportFuelCardsModal({ open, onClose }: ImportFuelCardsModalProps) {
  ...
  async function handleImport(): Promise<void> {
    ...
    setError(null);
    try {
      const response = await importFuelCards(selectedFile);
      setResult(response);
    } catch (currentError) {
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/45 px-4 py-6 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-[28px] border border-gray-200 bg-white shadow-[0_30px_90px_rgba(15,23,42,0.24)]">
        <div className="border-b border-gray-100 px-6 py-5">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#1D4E35]">Carte carburante</p>
          <h2 className="mt-2 text-2xl font-semibold text-gray-900">Carica file Excel carte</h2>
          <p className="mt-1 text-sm text-gray-500">
            L&apos;import aggiorna anagrafica carte (PAN) e traccia i passaggi tra operatori quando cambia il campo Driver.
          </p>
        </div>
        ...
        <div className="space-y-5 px-6 py-6">
          ...
          >
            ...
            <p className="mt-2 text-sm text-gray-500">
              Formato atteso: `Codice`, `Driver`, `Sigla`, `Bloccata`, `PAN`, `N. Carta/Emissione`, `Data Scadenza`, `Prodotti`, `COD`.
            </p>
          ...
          <div className="rounded-[24px] border border-[#d9dfd6] bg-[#fbfcfa] px-4 py-4 text-sm text-gray-600">
            Matching operatori: colonna `Driver` → `WCOperator.last_name + first_name` (varianti con virgola o parentesi vengono normalizzate).
            I cambi carta vengono tracciati come storico con data = momento import.
          </div>
            ...
            <div className="rounded-[24px] border border-[#d9dfd6] bg-[#fbfcfa] p-5">
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-4">
                <div className="rounded-2xl border border-[#e4e8e2] bg-white px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.18em] text-gray-500">Nuove</p>
                  <p className="mt-2 text-2xl font-semibold text-[#183325]">{result.imported}</p>
                </div>
                <div className="rounded-2xl border border-[#e4e8e2] bg-white px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.18em] text-gray-500">Aggiornate</p>
                  <p className="mt-2 text-2xl font-semibold text-[#183325]">{result.updated}</p>
                </div>
                <div className="rounded-2xl border border-[#e4e8e2] bg-white px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.18em] text-gray-500">Passaggi</p>
                  <p className="mt-2 text-2xl font-semibold text-[#183325]">{result.assignments_created}</p>
                </div>
                <div className="rounded-2xl border border-[#e4e8e2] bg-white px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.18em] text-gray-500">Driver non matchati</p>
                  <p className="mt-2 text-2xl font-semibold text-[#183325]">{result.unmatched_drivers}</p>
                </div>
              </div>
