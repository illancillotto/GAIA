...
type ElaborazioneCaptchaDialogProps = {
  open: boolean;
...
export function ElaborazioneCaptchaDialog({
  open,
  imageUrl,
  requestLabel,
  busy = false,
  onSolve,
  onSkip,
}: ElaborazioneCaptchaDialogProps) {
      ...
      <div className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
        <div className="mb-4">
          ...
          <p className="section-copy">
            {requestLabel ?? "Inserisci il codice mostrato per far ripartire l'elaborazione."}
          </p>
        </div>

        <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
          <Image
            alt="CAPTCHA elaborazioni"
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex items-center justify-end gap-3">
            <button className="btn-secondary" disabled={busy} onClick={() => void onSkip()} type="button">
              Salta
            </button>
            <button
