...
export function ModuleWorkspaceHero({
  ...
  return (
    <section
      className={cn(
        "overflow-hidden rounded-[28px] border border-[#d8dfd3] bg-[radial-gradient(circle_at_top_left,_rgba(212,231,220,0.95),_rgba(248,246,238,0.92)_55%,_rgba(255,255,255,0.98)_100%)] shadow-panel",
        compact ? "p-5" : "p-6",
      )}
    >
        ...
        <div>
          <div
            className={cn(
              ...
              compact ? "px-3 py-1 text-[10px]" : "px-3 py-1 text-[11px]",
            )}
          >
            {badge}
          </div>
...
export function ModuleWorkspaceMiniStat({
  eyebrow,
  value,
  description,
  tone = "default",
...
export function ModuleWorkspaceNoticeCard({
  title,
  description,
  tone = "neutral",
  compact = false,
