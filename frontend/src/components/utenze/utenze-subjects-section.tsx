...
export function UtenzeSubjectsSection({ token }: { token: string }) {
  ...
  useEffect(() => {
    if (!selectedSubjectId && !previewDocument) {
      return;
    }
  ...
  }, [previewDocument, previewUrl, selectedSubjectId]);
  ...
  const openHistoryModal = useCallback(async (e: MouseEvent, subjectId: string, displayName: string) => {
    e.stopPropagation();
    ...
    <div className="page-stack">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="section-title">Soggetti e import anagrafica</p>
          <p className="section-copy">
            Registro ricercabile, export, import CSV e creazione delle schede non ancora presenti sul NAS.
          </p>
        </div>
      ...
      {saveMessage ? (
        <p className="-mt-1 mb-2 text-sm text-[#1D4E35]">{saveMessage}</p>
      ) : null}
