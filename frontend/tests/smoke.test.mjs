...
test("catasto pages wire api client and realtime workflow", () => {
  ...
  assert.match(settingsPage, /Credenziali e test realtime del canale visure/);
  assert.match(settingsPage, /Gestione del pool account inVOLTURE/);
  ...
  assert.match(settingsPage, /CapacitasTestDialog/);
  assert.match(settingsPage, /Dettaglio backend/);
  assert.match(settingsPage, /Diagnosi operativa/);
  ...
  assert.match(newBatchPage, /redirect\("\/catasto\/new\?mode=batch"\)/);
  assert.match(newSinglePage, /redirect\("\/catasto\/new\?mode=single"\)/);
  assert.match(requestWorkspace, /createCatastoBatch/);
  assert.match(requestWorkspace, /startCatastoBatch/);
  assert.match(requestWorkspace, /Scarica template CSV/);
  assert.match(requestWorkspace, /useForm/);
  assert.match(requestWorkspace, /createCatastoSingleVisura/);
  ...
  assert.match(read("src/app/catasto/batches/page.tsx"), /redirect\("\/catasto\/archive\?view=batches"\)/);
  ...
  assert.match(archiveWorkspace, /cancelCatastoBatch/);
  assert.match(archiveWorkspace, /retryFailedCatastoBatch/);
