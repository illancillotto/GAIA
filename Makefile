...
lint-backend:
	backend/.venv/bin/python -m compileall -q backend/app backend/tests modules/elaborazioni/worker

lint-frontend:
	cd frontend && npm run lint

complexity-report:
	python tools/code_quality/complexity.py report --json $${REPORT_JSON:-reports/code-quality/complexity-report.json} --markdown $${REPORT_MD:-reports/code-quality/complexity-report.md}

complexity-check:
	python tools/code_quality/complexity.py check

complexity-changed:
	python tools/code_quality/complexity.py changed --base-ref $${BASE_REF:-origin/main}

complexity-baseline:
	python tools/code_quality/complexity.py baseline
...
quality-test:
	backend/.venv/bin/python -m pytest -q tests/code_quality/test_complexity_tool.py
