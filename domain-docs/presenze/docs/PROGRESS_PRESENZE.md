...
- verifica 2026-07-10: riallineate le fixture test frontend Presenze ai tipi correnti (`operai_group`, `gate_mobile_console`, campi operativi giornaliera) e rimosse cancellazioni opzionali non compatibili con il typecheck;
- `frontend npm run typecheck`: ok dopo il riallineamento fixture;
- `frontend npm run test:unit -- tests/unit/presenze-anomaly-months.test.ts tests/unit/presenze-collaborator-mapping.test.ts tests/unit/presenze-collaboratore-detail.test.tsx tests/unit/presenze-pages.test.tsx`: ok, 101 test;
- `frontend VITEST_COVERAGE_INCLUDE='src/lib/presenze-anomaly-months.ts,src/lib/presenze-collaborator-mapping.ts,src/lib/presenze-collaboratore-detail-helpers.ts,src/app/presenze/collaboratori/page.tsx,src/app/presenze/collaboratori/[id]/page.tsx,src/app/presenze/page.tsx,src/app/presenze/regole/page.tsx,src/app/presenze/squadre/page.tsx' npx vitest run --coverage --coverage.thresholds.lines=0 --coverage.thresholds.functions=0 --coverage.thresholds.statements=0 --coverage.thresholds.branches=0 tests/unit/presenze-anomaly-months.test.ts tests/unit/presenze-collaborator-mapping.test.ts tests/unit/presenze-collaboratore-detail.test.tsx tests/unit/presenze-pages.test.tsx`: ok come diagnostica senza gate; perimetro misurato `99.62%` statement / `97.56%` branch / `100%` functions / `99.67%` lines, con residui su runtime preesistente non modificato;
- aggiunti endpoint LAN autenticati `/api/mobile-sync/presenze/{teams,months,giornaliere,anomalie,rules}/snapshot` per permettere al connector GAIA in intranet di leggere gli stessi payload snapshot e ripubblicarli su GATE in outbound, senza chiamate GATE cloud -> GAIA LAN;
- deploy CED verificato 2026-07-10: `giornaliere/snapshot` e `anomalie/snapshot` pubblicati verso GATE con risposta `200`; aggiunta compatibilita con `sync/plan` legacy e alias payload `records/giornaliere` e `anomalies/anomalie`;
- verifica 2026-07-27: aggiunti alias canonici LAN `/api/mobile-sync/presenze/{teams,months,giornaliere,anomalie,rules}` senza suffisso `/snapshot`, endpoint idempotente `POST /api/mobile-sync/mobile-devices` e serializzazione batch degli snapshot mensili per evitare timeout; deploy CED verificato dal connector GATE con luglio 2026: `2325` giornaliere e `297` anomalie pushate, heartbeat `gaia-lan-1` online e ciclo `failed=false`;
- verifica 2026-07-16: aggiunta abilitazione controllata console GATE per operatori WhiteCompany collegati a operai/impiegati presenti nelle giornaliere; prova applicata con `limit=1` su `cadoni.marco` come `viewer`, con candidati residui pari a `0` nel DB locale;
...
- verifica 2026-08-31: gli snapshot `months`, `giornaliere` e `anomalie` espongono l'oggetto globale additivo `inaz_sync`, risolto esclusivamente dai job live persistiti in `presenze_sync_jobs`; `last_success_at` e `data_updated_at` avanzano solo dopo una coorte completa senza errori, un fallimento successivo mantiene il successo precedente con stato `degraded`, e gli errori pubblici non includono il dettaglio grezzo del worker. Contratto e semantica sono documentati in `GATE_MOBILE_INAZ_SYNC_CONTRACT.md`;
- deploy CED 2026-08-31: ricostruita l'immagine condivisa da `backend` e `presenze-worker`, entrambi attivi dopo il restart; i tre endpoint autenticati hanno risposto `200` con `inaz_sync` completo e stato reale `degraded`; un ciclo connector end-to-end ha pubblicato con `200` gli snapshot `months`, `giornaliere` e `anomalie`, senza migrazioni o cambi configurativi. Evidenze in `reports/2026-08-31-gate-mobile-inaz-sync-status-deploy.md`;
