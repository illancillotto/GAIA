...
- I run AdE del comprensorio non devono più dipendere da `BackgroundTasks` del backend: `sync-bbox-async` crea un record `queued` in `cat_ade_sync_runs` e il container `gaia-elaborazioni-worker` lo preleva dalla coda, lo porta in `processing` e ne aggiorna il progresso fino a `completed/failed`.
...
- Per codice nuovo o modificato nel dominio `catasto`, adottare una policy test pragmatica: coverage minima `>= 80%` per classe/file toccato, con target preferenziale `>= 90%` per parser, validatori, normalizzatori e servizi puri. Non usare la coverage globale di repository come unico criterio di qualita.
