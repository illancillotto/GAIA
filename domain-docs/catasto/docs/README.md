...
- Nella modal di dettaglio particella (lista `catasto/particelle`, ricerca anagrafica, pannello GIS), lo stesso blocco utenze deve consentire l’apertura del quick view soggetto (`UtenzeSubjectQuickViewDialog`, iframe `/utenze/{id}?embedded=1`) e il link certificato Capacitas con la stessa logica di contesto occupazione usata in `particelle/[id]`.
...
- Nella pagina `catasto/gis` la vista iniziale deve privilegiare i `Distretti` come contorno territoriale, ma senza riempimento attivo per non confonderli con una selezione utente; il layer `Particelle` resta spento di default per evitare rumore visivo sull'intero comprensorio. Il filtro `Distretto` deve restare disponibile sia nella sidebar standard sia nella `Vista estesa`, cosi l'operatore puo isolare un distretto e decidere solo dopo se accendere il dettaglio particellare.
...
- I confini distretto in mappa devono usare la vista MVT `cat_distretti_boundaries`, derivata da `ST_Boundary(ST_UnaryUnion(...))`, cosi l'outline non disegna i bordi interni delle particelle o dei frammenti poligonali del distretto.
