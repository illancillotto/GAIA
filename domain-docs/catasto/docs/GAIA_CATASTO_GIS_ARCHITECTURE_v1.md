...
La console `catasto/gis` espone il pannello `Allinea comprensorio AdE` nella sidebar standard e in vista estesa. L'azione principale calcola la bbox dai distretti attivi del comprensorio, avvia `sync-bbox`, riceve il `run_id`, chiama il report e mostra i contatori principali. Le bbox manuali, da area disegnata o da singolo distretto restano disponibili come scope avanzato per test mirati.
...
- **Pannello distretti**: accordion `Distretti irrigui` alimentato da `/catasto/distretti`; include ricerca per numero/nome, applica il filtro `num_distretto`, centra la geometria via `/catasto/distretti/{id}/geojson` e usa la stessa palette colore del layer MVT.
