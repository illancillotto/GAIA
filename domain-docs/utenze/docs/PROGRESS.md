...
- `elaborazioni-worker-runtime` accoda automaticamente un solo sweep REGISTRY giornaliero dopo le `22:00 Europe/Rome` (configurabile con `UTENZE_NAS_AUTO_IMPORT_TIME` e `UTENZE_NAS_AUTO_IMPORT_TIMEZONE`) quando non esiste gia un job `pending`/`running`. Gli sweep automatici registrano solo i nuovi `nas_path`, senza riscrivere documenti gia noti o produrre audit vuoti; l'attore audit resta nullo e il trigger `worker_schedule` e tracciato nel job. Vista rapida e pagina dettaglio soggetto espongono inoltre l'azione immediata `Aggiorna documenti`, limitata alla cartella NAS del soggetto.
...
- dettaglio soggetto Utenze: la tab `Avvisi di pagamento` mostra anche i ruoli storicizzati del soggetto e, dentro la card del relativo ruolo, l'evidenza PEC/raccomandata (stato, data e tracking). La raccomandata non viene collegata direttamente all'anagrafica: deriva sempre dall'associazione canonica con `ruolo_avvisi`, evitando duplicazioni e fallback anagrafici.
- dettaglio soggetto Utenze: aggiunta la tab `Terreni e colture`, alimentata in sola lettura dal modulo Ruolo. Mostra particelle, colture, superfici, comuni/distretto e warning di mapping catastale; la vista GIS carica automaticamente le geometrie presenti nel ruolo all'apertura della tab e mantiene il pulsante solo come ricarica manuale. La sezione usa la dicitura `a ruolo`: non certifica proprieta civilistica aggiornata e non avvia nuove sincronizzazioni.
- dettaglio soggetto Utenze: la vista GIS `Terreni e colture` gestisce anche chiusura/cambio soggetto durante il caricamento GeoJSON, ignorando risposte tardive per evitare aggiornamenti su modal non piu attive.
- dettaglio soggetto Utenze: la card `Visura per soggetto` mostra l'ultima visura Catasto scaricata con data/ora e azione `Visualizza visura`. La richiesta non viene piu bloccata per visure recenti: se l'ultima visura ha meno di 7 giorni appare una conferma esplicita prima di accodare una nuova richiesta SISTER.
...
- `VITEST_COVERAGE_INCLUDE=src/components/utenze/utenze-terreni-colture-section.tsx npm run test:coverage -- tests/unit/utenze-terreni-colture-section.test.tsx` ✅ (`100%` statements/branches/functions/lines, `8 passed`)
...
- `cd backend && PYTHONPATH=/home/cbo/CursorProjects/GAIA/backend ../.venv/bin/pytest tests/test_anagrafica_*.py tests/test_utenze_content_classification_service.py -q` ✅ (`76 passed`)
...
- `cd frontend && npm run test:unit -- tests/unit/utenze-subject-visura-card.test.tsx` ✅ (`7 passed`)
- `cd frontend && VITEST_COVERAGE_INCLUDE=src/components/utenze/utenze-subject-visura-card.tsx npm run test:coverage -- --run tests/unit/utenze-subject-visura-card.test.tsx` ✅ (`100% statements/branches/functions/lines`)
- `cd frontend && npm run typecheck` ✅
- `make graphify-frontend`, `make graphify-utenze-code`, `make graphify-utenze-docs` ✅
- visibilita raccomandate su dettaglio soggetto: backend route coverage `100%` (`129/129` statement, `14/14` branch); frontend coverage `100%` (`389/389` statement, `402/402` branch, `85/85` funzioni, `305/305` linee); suite frontend mirata `33 passed`; TypeScript ✅
