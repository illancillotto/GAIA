# GAIA Inaz Giornaliere - Specifica modulo

> Fonte tecnica iniziale: repository locale `/home/cbo/CursorProjects/inaz-scraper`.
...
Il modulo `inaz` deve permettere a GAIA di leggere, normalizzare, archiviare e consultare le giornaliere lavorative degli utenti applicativi. La sorgente esterna e il portale Inaz `https://serviziweb.inaz.it/portalecbo/default.aspx`, oggi gestito dal tool Python `inaz-scraper` tramite Playwright perche il portale usa iframe, pagine ASP.NET e funzioni JavaScript lato pagina.
...
- importare giornaliere da file JSON/CSV/XLSX generati da `inaz-scraper`;
...
Nel repo `inaz-scraper` sono presenti tre entrypoint:

- `inaz-straordinari`: login/navigazione e scraping righe potenzialmente legate a straordinari;
- `inaz-crea-mesi`: crea file Excel mensili partendo da `Straordinari.xlsx`;
- `inaz-popola-excel`: legge giornate dal calendario Inaz e popola i file mensili.
...
`inaz-straordinari` produce righe generiche:
...
`inaz-popola-excel` produce record piu vicini al dominio giornaliera:
...
frontend/src/components/inaz/
frontend/src/lib/inaz-api.ts
frontend/src/types/inaz.ts
```

Documentazione:

```text
domain-docs/inaz/docs/
```
...
1. Import file: JSON/CSV/XLSX generato o compatibile con `inaz-scraper`. E il percorso MVP piu sicuro.
...
- non salvare `.inaz-storage-state.json` in repository;
...
- `module_inaz boolean not null default false`.
...
Per `super_admin`, includere `inaz` nella lista moduli sempre abilitati.
...
| `source` | string | `manual`, `file_import`, `inaz_sync` |
...
Tutti sotto prefisso `/inaz` incluso da `backend/app/api/router.py`.

### 6.1 Giornaliere

| Metodo | Path | Permesso | Descrizione |
| --- | --- | --- | --- |
| `GET` | `/inaz/giornaliere` | `inaz.giornaliere` | lista paginata con filtri |
| `POST` | `/inaz/giornaliere` | `inaz.edit` | crea giornaliera manuale |
| `GET` | `/inaz/giornaliere/{id}` | `inaz.giornaliere` | dettaglio |
| `PUT` | `/inaz/giornaliere/{id}` | `inaz.edit` | aggiorna record manuale/correzione |
| `POST` | `/inaz/giornaliere/{id}/review` | `inaz.review` | marca reviewed/approved/rejected |
| `DELETE` | `/inaz/giornaliere/{id}` | `inaz.admin` | eliminazione controllata |
...
| Metodo | Path | Permesso | Descrizione |
| --- | --- | --- | --- |
| `GET` | `/inaz/users` | `inaz.giornaliere` | utenti GAIA con `module_inaz=true` |
| `GET` | `/inaz/users/{user_id}/calendar` | `inaz.giornaliere` | calendario mensile/periodo |
| `GET` | `/inaz/users/{user_id}/summary` | `inaz.giornaliere` | KPI periodo |

### 6.3 Import file

| Metodo | Path | Permesso | Descrizione |
| --- | --- | --- | --- |
| `POST` | `/inaz/import/upload` | `inaz.import` | upload JSON/CSV/XLSX e avvio job |
| `GET` | `/inaz/import/jobs` | `inaz.import` | lista job |
| `GET` | `/inaz/import/jobs/{job_id}` | `inaz.import` | dettaglio job |
| `POST` | `/inaz/import/preview` | `inaz.import` | validazione senza persistenza |
...
| Metodo | Path | Permesso | Descrizione |
| --- | --- | --- | --- |
| `POST` | `/inaz/sync/jobs` | `inaz.sync` | avvia sync Playwright |
| `GET` | `/inaz/sync/jobs` | `inaz.sync` | lista sync |
| `GET` | `/inaz/sync/jobs/{job_id}` | `inaz.sync` | stato sync |
| `POST` | `/inaz/sync/jobs/{job_id}/cancel` | `inaz.sync` | richiesta stop best-effort |
...
| Metodo | Path | Permesso | Descrizione |
| --- | --- | --- | --- |
| `GET` | `/inaz/exports/monthly.xlsx` | `inaz.export` | export Excel mensile |
| `GET` | `/inaz/exports/giornaliere.csv` | `inaz.export` | export CSV |
| `GET` | `/inaz/exports/summary.json` | `inaz.export` | summary machine-readable |
...
Usare `require_module("inaz")` per gate di modulo e `require_section(...)` dove serve controllo fine.
...
- `/inaz/settings`: credenziali/sessioni, solo admin.
...
- creare `domain-docs/inaz/docs`;
- aggiungere `module_inaz` a `application_users` con migrazione Alembic;
- aggiornare model, schema, repository, bootstrap admin, frontend types e admin users UI;
- aggiungere sezioni bootstrap `inaz.*`;
- creare router vuoto `/inaz/health` o dashboard summary minima.

### Fase 2 - Data model e API base

- creare modelli ORM `InazDailyRecord`, `InazDailyInterval`, `InazImportJob`;
...
- supportare CSV generato da `inaz-straordinari`;
...
- estrarre dal repo `inaz-scraper` un client interno o pacchetto riusabile;
- introdurre `InazSyncJob`;
...
- `enabled_modules` include `inaz` quando `module_inaz=true`;
- super admin vede sempre modulo `inaz`;
...
- sidebar mostra modulo Inaz solo se abilitato;
...
Regole:
- nuovo codice backend sotto `backend/app/modules/inaz/`;
- nuove route sotto prefisso `/inaz`;
- aggiungi `module_inaz` a `application_users` con migrazione Alembic, model,
...
- `frontend/src/types/api.ts`
- `frontend/src/lib/api.ts` o nuovo `frontend/src/lib/inaz-api.ts`
