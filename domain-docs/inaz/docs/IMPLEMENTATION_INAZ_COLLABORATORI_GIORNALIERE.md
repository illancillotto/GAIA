...
> Stato: documento operativo per portare in GAIA il flusso gia implementato in `/home/cbo/CursorProjects/inaz-scraper`.
...
Nel repo `inaz-scraper` e stato aggiunto un flusso dedicato ai capisettore:

- file principale: `src/inaz_scraper/collaborators.py`;
- comando CLI: `inaz-collaboratori`;
- entrypoint in `pyproject.toml`: `inaz_scraper.collaborators:main`;
...
```bash
cd /home/cbo/CursorProjects/inaz-scraper
PYTHONPATH=src .venv/bin/python -m inaz_scraper.collaborators \
  --year 2026 \
  --month 5 \
  --limit 1 \
  --json-output /tmp/inaz_test_giornaliere.json \
...
```text
/home/cbo/CursorProjects/inaz-scraper/Giornaliere/Giornaliere_2026_803_1.xlsm
```
...
```text
backend/app/modules/inaz/services/sync_service.py
```
...
| Metodo | Path | Scopo |
| --- | --- | --- |
| `GET` | `/inaz/collaborators` | lista collaboratori Inaz |
| `PUT` | `/inaz/collaborators/{id}/application-user` | collega collaboratore a `application_users` |
| `GET` | `/inaz/giornaliere` | lista giornaliere con filtri |
| `GET` | `/inaz/giornaliere/{id}` | dettaglio giornaliera |
| `GET` | `/inaz/collaborators/{id}/calendar` | calendario mese |
| `GET` | `/inaz/collaborators/{id}/summary` | riepilogo eventi |
| `POST` | `/inaz/import/json` | importa JSON prodotto dallo scraper |
| `POST` | `/inaz/sync/jobs` | avvia job live Inaz |
| `GET` | `/inaz/sync/jobs/{id}` | stato job |
| `GET` | `/inaz/export/giornaliere.xlsm` | genera XLSM |

### 5.5 Permessi

Estendere quanto gia previsto in `GAIA_INAZ_GIORNALIERE_MODULE_SPEC.md`:
...
1. GAIA importa solo JSON prodotto da `inaz-collaboratori`.
...
sviluppato in `/home/cbo/CursorProjects/inaz-scraper/src/inaz_scraper/collaborators.py`.

Obiettivo MVP:
- importare in GAIA il JSON prodotto da `inaz-collaboratori`;
...
Regole:
- backend nel monolite modulare sotto `backend/app/modules/inaz/`;
- route sotto prefisso `/inaz`;
...
- aggiungere `module_inaz` a `application_users` solo se non gia presente;
