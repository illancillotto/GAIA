# WhiteCompany → GAIA — Piano di integrazione organico v7

> Stato: v7 — 10 aprile 2026 — BASELINE IMPLEMENTATIVA
...
>              path doc reale, semantica review_status, shape /sync/status,
>              warehouse_request DDL, regola applicativa rejected, step ridondante Fase 3,
>              path completi uniformati, sync/run body+response+coerenza job-per-entity,
>              namespace /api/utenze uniformato, migration Fase 6 separate,
>              UI vs API path Fase 5 e 7, sezione 4 migration riscritta pulita,
>              never come stato derivato chiarito, scope date_from/date_to esplicitato
>
...
CREATE TABLE wc_sync_job (
    ...
    -- Nota: "never" NON è un valore DB. È uno stato derivato restituito
    -- da GET /sync/status quando non esiste ancora alcun job per quell'entity.
...
```
1. 20260411_NNNN_add_wc_sync_job.py
      → wc_sync_job

2. 20260411_NNNN_add_wc_area_and_operator.py
      → wc_area
      → wc_operator
      → campi wc_* su field_report_category, vehicle, vehicle_fuel_log, vehicle_usage_session
      → wc_area_id su field_report (nullable FK)

3. 20260411_NNNN_add_bonifica_user_staging.py
      → bonifica_user_staging

4. 20260411_NNNN_add_wc_org_charts.py
      → wc_org_chart
      → wc_org_chart_entry
      (dipende da wc_area e wc_operator — applica dopo migration 2)

5. 20260411_NNNN_add_warehouse_request.py
      → warehouse_request
      (dipende da field_report — applica dopo migration 2)
```
...
POST /elaborazioni/bonifica/sync/run
     body: {
       "entities": ["reports", "vehicles", ...] | "all",
       "date_from": "YYYY-MM-DD",   // opzionale
       "date_to":   "YYYY-MM-DD"    // opzionale
     }
     auth: admin
     // Crea un wc_sync_job per ciascuna entity richiesta (status="running").
     // Non esiste un job padre batch: ogni entity è indipendente e tracciabile
     // singolarmente in wc_sync_job e in GET /sync/status.
     // Regola date_from/date_to:
     //   - Si applicano SOLO alle entity date-aware:
     //     "reports", "refuels", "taken_charge", "warehouse_requests".
     //   - Le entity non date-aware ("report_types", "vehicles", "areas", "operators",
     //     "users", "org_charts") ignorano i parametri data e sincronizzano sempre tutto.
     //   - Se omessi, le entity date-aware usano una finestra di default (es. ultimi 30 giorni),
     //     configurabile via variabile d'ambiente WC_SYNC_DEFAULT_DAYS.
     response: {
       "jobs": {
         "reports":      { "job_id": "uuid", "status": "running", "started_at": "..." },
         "report_types": { "job_id": "uuid", "status": "running", "started_at": "..." },
         "vehicles":     { "job_id": "uuid", "status": "running", "started_at": "..." }
         // una chiave per ogni entity avviata
       }
     }
...
28. API:
    ...
    `POST /api/utenze/bonifica-staging/{id}/reject`
    `POST /api/utenze/bonifica-staging/bulk-approve`
29. UI: pagina `/utenze/bonifica-staging` — lista staging con filtri review_status, azioni approve/reject
30. Bulk approve per `review_status=new`
31. Singola approve/reject per `review_status=mismatch`

### Fase 6 — Magazzino e organigrammi
32. Migration `add_wc_org_charts` (dipende da `wc_area` e `wc_operator`, già applicate in Fase 1)
33. Migration `add_warehouse_request` (dipende da `field_report`, già presente)
...
36. `backend/app/modules/elaborazioni/bonifica_oristanese/apps/warehouse_requests/client.py`
37. `backend/app/modules/inventory/services/sync_warehouse.py`
38. API inventory warehouse requests
39. Background job: link `warehouse_request.field_report_id` via `wc_report_id`

### Fase 7 — Frontend sync dashboard
40. Blocco stato sync in `/elaborazioni/settings` (workspace Bonifica già presente)
41. Pagina `/elaborazioni/bonifica/sync` con tabella `wc_sync_job` per entità
    (usa `GET /elaborazioni/bonifica/sync/status`)
42. UI revisione consorziati: pagina `/utenze/bonifica-staging` (consuma API `/api/utenze/bonifica-staging`)
